...
function TopRatedPage() {
  ...
  useEffect(() => {
    const mainEndPoint = `${API_URL}movie/top_rated?api_key=${API_KEY}&language=ko-KR&page=1&region=KR`;
  ...
  const loadMoreItems = () => {
    const listEndPoint = `${API_URL}movie/top_rated?api_key=${API_KEY}&language=ko-KR&page=${CurrentPage + 1}&region=KR`;
  ...
  return (
    <div style={{ width: "100%", margin: "0" }}>
      {/* Main Image*/}
      <MainImage image={`${IMG_URL}w1280${MainMovie.backdrop_path}`} title={MainMovie.original_title} description={MainMovie.overview} page="top_rated" />
...
export default Auth(TopRatedPage, null);
